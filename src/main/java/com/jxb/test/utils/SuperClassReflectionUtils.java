...
public class SuperClassReflectionUtils {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SuperClassReflectionUtils.class);
        ...
        for(; clazz != Object.class ; clazz = clazz.getSuperclass()) {
            try {
                ...
                for (int i = fields.length-1; i < fields.length; i--) {
                    ...
                    if(fields[i].get(object) == null || "serialVersionUID".equals(fields[i].getName())){
                        continue;
                    }
                    //得到此属性的值
                    //logger.info("name:" + fields[i].getName() + "\t value = " + fields[i].get(object));
